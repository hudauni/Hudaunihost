...
import { Check, X, Trash2, Clock, User, Hash, CreditCard, Loader2, BookOpen, Phone } from 'lucide-react';
...
export default function AdminCourseInbox() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const data = snap.docs.map(d => ({ id: d.id, ...d.data() })) as CourseRequest[];
      setRequests(data);
    } catch (e) {
      console.error("Error fetching course requests:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatus = async (id: string, status: 'approved' | 'declined') => {
    try {
      await updateDoc(doc(db, "courseRequests", id), { status });
      fetchRequests();
      showAlert('success', status === 'approved' ? 'অনুমোদিত' : 'প্রত্যাখ্যাত', `অনুরোধটি সফলভাবে ${status === 'approved' ? 'অ্যাপ্রুভ' : 'ডিক্লাইন'} করা হয়েছে।`);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="space-y-10">
      ...
      />
      <div>
        <h2 className="text-3xl font-bold text-white mb-2 font-bengali">কোর্স ইনবক্স</h2>
        <p className="text-white/40 text-sm">Manage course enrollment verification requests</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {requests.length === 0 ? (
          <div className="py-20 text-center text-white/10 border-2 border-dashed border-white/5 rounded-xl font-bengali bg-white/5">
            কোনো কোর্স রিকোয়েস্ট পাওয়া যায়নি।
          </div>
        ) : requests.map((req) => (
          <div
            key={req.id}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 flex-1">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] text-white/40 uppercase font-bold tracking-widest">Student</p>
              ...
              {/* 2. Course Info */}
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center text-emerald-400/40 shadow-inner"><BookOpen size={20}/></div>
                <div className="min-w-0">
                  <p className="text-[10px] text-white/40 uppercase font-bold tracking-widest">Course</p>
